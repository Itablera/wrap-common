...
import { PlainValue, PlainValueTypes, ResultOrError } from '../types';
...
export function ConvertToPlainValue(value:PlainValue, type: PlainValueTypes): ResultOrError<PlainValue, string> {
    switch(type) {
        case 'string':
            return ResultOrError.try(String(value), 'Could not convert value to string');
        case 'number':
            return ResultOrError.try(Number(value), 'Provided input is not a number');
        case 'boolean':
            ResultOrError.ok(value === 'true');
        default:
            ResultOrError.fail('Unknown type or format')
        }
}